...
- (void)commonInit {
    ...
    UIToolbar* numberToolbar = [[UIToolbar alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, 44)];
...
- (void)deleteTagButton:(UIButton *)sender {
    ...
    if ([self.tapDelegate respondsToSelector:@selector(tagsControl:didRemoveTag:)]) {
        [self.tapDelegate tagsControl:self didRemoveTag:[_tags objectAtIndex:index]];
    }
...
-(void)toolbarDoneClicked{
    if (tagInputField_.text.length == 0){
        ...
        tagInputField_.text = @"";
        if ([self.tapDelegate respondsToSelector:@selector(tagsControl:didAddTag:)]) {
